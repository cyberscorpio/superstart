/** vim: ts=8 filetype=javascript:
 * modified from resource:///modules/BrowserNewTabPreloader.jsm
 */
"use strict";

this.EXPORTED_SYMBOLS = ["SuperStartPreloader"];
...
const XUL_PAGE = "data:application/vnd.mozilla.xul+xml;charset=utf-8,<window%20id='ss-win'/>";
const PREF_BRANCH = "extensions.superstart.";

let NEWTAB_URL = "chrome://superstart/content/index.html";
...
this.SuperStartPreloader = {
	init: function Preloader_init(url) {
		NEWTAB_URL = url;
		Initializer.start();
	},
...
Object.freeze(SuperStartPreloader);
...
let Preferences = {
	...
	get enabled() {
		if (this._enabled === null) {
			this._enabled = this._branch.getBoolPref("page.preload");
		}
...
let HiddenBrowsers = {
	...
	get: function (width, height) {
		...
		Cu.reportError("SuperStartPreloader: no matching browser found after updating");
		for (let [size, browser] of this._browsers) {
			return browser;
		}

		// We should really never be here.
		Cu.reportError("SuperStartPreloader: not even a single browser was found?");
		return null;
	},
...
HiddenBrowser.prototype = {
	...
	_createBrowser: function () {
		HostFrame.get().then(aFrame => {
			...
			this._applySize();
			doc.getElementById("ss-win").appendChild(this._browser);

			this._browser.docShell.isActive = false;
			this._browser.messageManager.loadFrameScript(BROWSER_CONTENT_SCRIPT, true);
		});
	},
