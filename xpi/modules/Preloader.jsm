 ...
 * modified from: resource:///modules/CustomizationTabPreloader.jsm
 */
"use strict";
...
const CUSTOMIZATION_URL = "about:superstart";
...
this.SuperStartPreloader = {
  uninit: function () {
    SuperStartPreloaderInternal.uninit();
  },

  newTab: function (aTab) {
    return SuperStartPreloaderInternal.newTab(aTab);
  },
  ...
  ensurePreloading: function() {
    SuperStartPreloaderInternal.ensurePreloading();
  },
};

Object.freeze(SuperStartPreloader);

this.SuperStartPreloaderInternal = {
  _browser: null,
  ...
  newTab: function (aTab) {
		  Cu.reportError('preload.newtab: ' + aTab);
...
if (wins.hasMoreElements()) {
	...
	if (win.gBrowser) {
		...
		let {w, h} = utils.getBoundsWithoutFlushing(win.gBrowser);
		this._browser.style.width = w + "px";
		this._browser.style.height = h + "px";
	}
}
