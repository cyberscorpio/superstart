...
Cu.import("resource://gre/modules/FileUtils.jsm");
	...
	this.changeSite = function(idx, url, name, custImg) {
		...
		if (url == site.url) {
			...
			this.removeSite(idx); // TODO: if this site is the last one of a row, maybe cause problem...
			if (url == null) {
				return;
			}
	...
	this.refreshSite = function(idx) {
		...
		if (idx < 0 || idx >= l) {
			logger.logStringMessage('[superstart] refreshSite(' + idx + ') out of range (' + l + ')');
			return;
		}

		let s = sites[idx];
		if (s.url == null) {
			return; // should only happen under refreshSite(-1)
		}
